...
##### Getting a photo back off the app

`saveMedia()` in `media.js`, reached from the disc in the top-left of
the lightbox. The app was a one-way street for media — you attached a
photo to a completion and from then on it was viewable and not
retrievable — which is the wrong shape for a record of your own life,
since the phone it was taken on gets replaced and the camera-roll copy
goes with it.

**The bytes are fetched, not linked, and that is the whole trick.**
Neither of the two obvious builds works: a cross-origin `<a download>`
is ignored (see above), and inside the Capacitor WKWebView *no*
script-driven download arrives at all. So the object is read into a
Blob and handed to **`deliverFile()`** (`utils.js`), whose first rung is
`navigator.share()` with a `File` — which on iOS is where *Save Image*
and *Save Video* live, so the share sheet is the route to the camera
roll as well as to Files.

- **`deliverFile()` is shared with `export.js`** — the two rungs that
  are about *this platform* live in one place, and each caller keeps
  its own floor below them, because the floors do not generalise: the
  export's is the JSON on screen with Copy, a photo's is opening the
  Worker URL. See the header of `export.js`.
- **The read goes through the Worker, not straight at the bucket.**
  ...
  way that looks exactly like the file being missing. The cost is that
  saving re-downloads rather than reusing what the `<img>` already
  pulled: the service worker caches that under a **no-cors** request, so
  the cached copy is opaque and `.blob()` cannot read it.
- **So a save needs the network — except for a legacy inline photo**,
  which is a `data:` URL and needs no request at all. Offline it says
  so rather than failing silently.
- **An `AbortError` is a success.** It is the user dismissing the share
  sheet; falling through to a download would hand them the file they
  just declined.
- **No toast after the share sheet.** It has just been on screen and
  gave its own confirmation, and a dismissal and a save are
  indistinguishable from here — saying "Saved" over a cancelled sheet
  would state something that did not happen. A blob download, which is
  silent, does get one.
- **A failed *fetch* falls to the floor too, but only while online.**
  A navigation needs no CORS, so handing the browser the Worker URL can
  still work where reading it could not. Offline there is nothing
  underneath and it says so instead.
- **The file is named after the activity** (`fileSlug()` in `utils.js`),
  with an index appended only when there is more than one item — or a
  single photo comes out called `kayaking-1`. The extension comes from
  the blob's own type first, because an inline photo has no extension to
  read.
- **The control is in the lightbox and nowhere else.** Every photo in
  the app passes through it — the stage, the hero and the grid on an
  accomplished activity all open it — so one button covers all of them
  rather than three call sites that would then have to agree. It takes
  the opposite corner from Close, so dismissing and keeping a copy are
  nowhere near each other, and it carries a **busy state** rather than a
  message beside it, because a video is megabytes and the fetch is not
  instant.


**And the whole set in one action.** `saveAllMedia()` in `media.js`,
behind the **Save all N** button under the media on a completed
activity's sheet (`saveActivityMedia()` in `activities.js`). Eleven taps
through eleven share sheets is not a feature.

- **⚠️ IT IS ONE SHARE SHEET, NOT A LOOP OF THEM.** `deliverFiles()`
  takes the whole set, so iOS offers *Save 11 Images* once. That is the
  only reason this is not `saveMedia()` in a `for`, and it is why the
  ladder in `utils.js` is plural at the bottom rather than wrapped in a
  loop at the top. Only the download rung is a loop, because a download
  genuinely is one file at a time — with a beat between them, or the
  browser drops everything after the first.
- **The fetches are pooled three at a time** (`MEDIA_SAVE_POOL`).
  Serial is eleven round trips end to end on a control somebody is
  watching; all at once competes with itself and, with video in the set,
  puts a lot of memory in flight.
- **⚠️ A FAILURE COSTS ONE ITEM, NOT THE SET.** Anything unreachable is
  counted and dropped and the rest is still delivered — the alternative
  is one bad photo standing between somebody and the other ten. The
  files keep the index they were shown at, so the names still run in
  order with a gap rather than being silently renumbered. **The count is
  reported**: handing over ten when eleven were asked for and saying
  nothing is the quiet kind of wrong this app tries not to ship.
- **One item routes through `saveMedia()` and must keep doing so**,
  because that path has the open-the-URL floor under it and this one
  cannot — there is no sensible way to open eleven URLs.
- **⚠️ THE BUTTON IS ON THE PHOTOS, NOT UNDER THEM** — a 44px rounded
  square in the bottom-right corner of `.ad-stage`, the way a camera app
  puts its controls on the frame (`adStageSaveHTML()`). It was a
  labelled mono row beneath the media; the glyph alone in the corner it
  acts on says the same thing in a quarter of the space, and the stage
  is the only place on the sheet where *these photos* is unambiguous.
  It is drawn for a single item too, which is **not** a duplicate of the
  lightbox disc: this is the only one of the two reachable without
  opening a photo first.
- **⚠️ IT IS SOLID `--tint`, NOT THE GLASS DISC THE LIGHTBOX AND THE MAP
  USE.** Those float over a ground the app chose; this one floats over
  whatever photograph is underneath, and a translucent control
  disappears into a light one. A rounded square rather than a circle for
  the same reason — it must not read as part of the picture. The fill is
  the one the Edit button in the dock below it already uses.
- **⚠️ ITS `onclick` MUST STOP PROPAGATION.** The whole stage is a
  button that opens the lightbox, so without it every save opened the
  viewer on top of the share sheet it had just raised.
- **⚠️ `.ad-dots` CARRIES SYMMETRIC SIDE PADDING BECAUSE OF THIS BUTTON**
  — the button's whole footprint, on both sides, because a centred row
  of dots grows towards both corners. Symmetric so they stay centred on
  the image rather than on what is left of it, and `flex-wrap` is the
  backstop: nineteen dots need 241px and a 320px phone has 180px to
  give, so without it they run out of the frame instead of onto a second
  row.
- **The progress is in the button** — the glyph is swapped for a `12/19`
  tally while it fetches. **Two classes, not one**: `is-busy` is the
  pulse and is always right, `counting` swaps the glyph and is only
  honest when there is a tally to show, or a single item would sit there
  saying `1/1`.
...
| `icons.js` | `ICON_PATHS`, the app's own inline-SVG glyph set (`download` is the newest — deliberately the `share` glyph with its arrow reversed), plus `ICON_FILLED` (glyphs already solid, which must not be stroked) and `icon(name, cls)`. Icons inherit `currentColor`. **Add new glyphs here**, not inline in a template string. |
...
| `media.js` | Photos **and video**. `probeStorage()`/`storageReady()`, **`r2Ready`/`uploadToR2`/`uploadToSupabase`/`mediaDownloadUrl`** (uploads go to Cloudflare R2 through the Worker, with Supabase Storage as the fallback — see **Media**), **`saveMedia`/`saveAllMedia`/`mediaBlobFor`/`mediaSaveName`/`saveMediaFallback`/`MEDIA_SAVE_EXT`/`MEDIA_SAVE_POOL`** (getting one — or an activity's whole set in a single share sheet — back onto the phone; see **Getting a photo back off the app**), `uploadPhoto`/`uploadVideo`, `videoPoster` (grabs a still so thumbnails and map pins have an image), `handleMedia`, `rmMedia`, `mediaTileHTML`, `renderThumbs` (which ends in `updateMediaRequirement()` — the completion sheet's media rule, owned by `activities.js`), and the ordering set — `coverIndex`, `moveMedia`, `makeCover`, `openMediaMenu`. Also the photo→location offer: `needsLocationSuggestion`, `suggestLocationFromPhoto`, `acceptPhotoLocation`, `dismissPhotoLocation`, `resetLocationSuggestion` (see **Where the photo was taken**). Working list is the `upMedia` global. Replaced `photos.js`; see **Media** below. |
